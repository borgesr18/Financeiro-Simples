...
export async function updateAccount(formData: FormData) {
  ...
  if (error) {
    console.error('[updateAccount]', { id, name, type, institution, color_hex, icon_slug, currency, archived })
...
export async function deleteAccount(formData: FormData) {
  ...
  if (error) {
    console.error('[deleteAccount]', { id })
...
export async function createTransfer(formData: FormData) {
  ...
  const payload = [
    { user_id: user.id, account_id: from_id, date: when, description: desc, amount: -Math.abs(amount), category_id: null },
    { user_id: user.id, account_id: to_id,   date: when, description: desc, amount:  Math.abs(amount), category_id: null }
  ]

  const { error } = await supabase.from('transactions').insert(payload)
  if (error) {
    console.error('[createTransfer] payload:', payload)
    console.error('[createTransfer] supabase error:', error)
    throw new Error(error.message)
  }

  revalidatePath('/banking')
  revalidatePath('/')       // se sua lista de lançamentos estiver na home
  redirect('/banking')
}
