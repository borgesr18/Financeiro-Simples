...
function clampInt(v: FormDataEntryValue | null, min: number, max: number, fb: number) {
...
export async function createCard(fd: FormData) {
  ...
  try {
    const name = ((fd.get('name') ?? '') as string).trim() || 'Cartão'
    ...
    const due_day = clampInt(fd.get('due_day'), 1, 28, Math.min(28, closing_day + 7))

    // cria a account tipo 'credit'
...
export async function updateCard(id: string, fd: FormData) {
  ...
  try {
    const { data: current, error: getErr } = await supabase
      ...
      .eq('id', id)
      .single()
    if (getErr || !current) throw new Error('Cartão não encontrado')

    const name = ((fd.get('name') ?? '') as string).trim() || current.name
    const brand = strOrNull(fd.get('brand'))
    const last4 = strOrNull(fd.get('last4'))
    const institution = strOrNull(fd.get('institution'))
    const color_hex = strOrNull(fd.get('color_hex'))
    ...
    const closing_day = clampInt(fd.get('closing_day'), 1, 28, current.closing_day ?? 5)
    const due_day = clampInt(fd.get('due_day'), 1, 28, current.due_day ?? Math.min(28, closing_day + 7))
    const archived = ((fd.get('archived') ?? '') as string).toLowerCase() === 'on'

    const cardUpd: any = { name, limit_amount, closing_day, due_day, archived }
    cardUpd.brand = brand
    cardUpd.last4 = last4
    ...
    // espelha alguns campos na account (best-effort)
    try {
      const accUpd: any = { name }
      ...
      console.warn('[cards:updateCard] espelho account falhou (ignorado):', e)
    }
  } catch (e) {
    if (isRedirectError(e)) throw e
...
/* ARCHIVE (opcional; hoje preferimos lixeira) */
export async function archiveCard(id: string) {
  ...
  const { error } = await supabase
    ...
    .eq('user_id', user.id)
    .eq('id', id)
  if (error) throw new Error('Falha ao arquivar cartão')

  revalidatePath('/cards')
}

/* Ciclo da fatura a partir do dia de fechamento */
...
/* Gera/atualiza fatura (fecha ciclo, soma despesas) */
export async function generateStatement(cardId: string) {
  ...
  const { data: c, error: cErr } = await supabase
    ...
    .eq('id', cardId)
    .single()
  if (cErr || !c) throw new Error('Cartão não encontrado')
...
/* Marca fatura como paga (gera dupla de transações e atualiza status) */
export async function payStatement(statementId: string, payFromAccountId: string) {
  ...
  const { data: st, error: stErr } = await supabase
    ...
    .eq('id', statementId)
    .single()
  if (stErr || !st) throw new Error('Fatura não encontrada')
  ...
  const { data: card, error: cErr } = await supabase
    ...
    .eq('id', st.card_id)
    .single()
  if (cErr || !card?.account_id) throw new Error('Conta do cartão não encontrada')
