  ...
  return (
    <main className="p-6 space-y-4">
      <div className="flex items-center justify-between">
        ...
        <Link href="/add" className="px-4 py-2 bg-sky-500 hover:bg-sky-600 text-white rounded-lg">Novo lançamento</Link>
      </div>
        ...
        <div className="overflow-x-auto">
          <table className="min-w-[880px] w-full text-sm">
            ...
            <tbody>
              {rows.map(tx => {
                ...
                return (
                  <tr key={tx.id} className="border-b last:border-0">
                    <td className="py-3 px-4">{new Date(tx.date).toLocaleDateString('pt-BR')}</td>
              ...
              {rows.length === 0 && (
                <tr>
                  <td colSpan={5} className="py-6 text-center text-neutral-500">Nenhum lançamento encontrado.</td>
                </tr>
              )}
            </tbody>
          </table>
